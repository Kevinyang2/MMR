...
class PointGenerator(nn.Module):
    ...
    def forward(self, pymid):
        points = []  # build the output point set for all levels
        sizes = [p.size(1) for p in pymid] + [0] * (len(self.buffer) - len(pymid))  # pad missing levels with zeros
        # iterate over requested level sizes and cached buffers
